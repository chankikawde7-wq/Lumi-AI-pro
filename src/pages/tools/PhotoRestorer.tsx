...
import { ArrowLeft, Download, Loader2, Wand2, AlertCircle, ZoomIn } from "lucide-react";
...
export default function PhotoRestorer() {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          // Apply baseline restoration filters (defade)
          let filterString = "contrast(1.2) brightness(1.05)";
          
          if (options.colorize) {
            filterString += " saturate(1.4)";
          } else {
            filterString += " grayscale(100%)"; // Force B&W if not colorizing
          }
          
          if (options.removeScratches) {
            // Apply a slight blur to smooth out scratches/dust before sharpening
            ctx.filter = "blur(0.8px)";
            ctx.drawImage(img, 0, 0);
            ctx.globalAlpha = 0.8; // Blend with original to keep some texture
          }
          ...
          if (options.enhanceFaces || options.removeScratches) {
            applySharpen(ctx, canvas.width, canvas.height, 0.4);
          }
          
          if (options.colorize) {
            // Add a subtle warm overlay to simulate colorization on old photos
  ...
  const startEnhancing = async () => {
    ...
    try {
      ...
      if (originalFile.size > 2 * 1024 * 1024) { // Compress if larger than 2MB
  ...
  const handleDownload = (format: "png" | "jpg" = "jpg") => {
    ...
    a.href = enhancedUrl;
    a.download = `restored_photo_${Date.now()}.${format}`;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold flex items-center gap-3 text-slate-900 dark:text-white"><Wand2 className="w-8 h-8 text-amber-500" /> Photo Restorer</h1>
          </div>
          {status === "success" && (
            <div className="flex gap-3 mt-4 md:mt-0">
              ...
              <button onClick={() => handleDownload("png")} className="px-4 py-2 bg-amber-600 hover:bg-amber-700 transition-colors text-sm font-medium text-white rounded-lg flex items-center shadow-sm"><Download className="w-4 h-4 mr-2"/> Download PNG</button>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <h3 className="font-bold mb-4 uppercase text-sm text-slate-900 dark:text-white">Repair Settings</h3>
              
              <label className="flex items-center mb-3 text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                <input type="checkbox" checked={options.colorize} onChange={(e) => setOptions({...options, colorize: e.target.checked})} disabled={status === "processing"} className="mr-3 rounded text-amber-600 focus:ring-amber-500 w-4 h-4" /> 
                Colorize
              </label>
              <label className="flex items-center mb-3 text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                <input type="checkbox" checked={options.removeScratches} onChange={(e) => setOptions({...options, removeScratches: e.target.checked})} disabled={status === "processing"} className="mr-3 rounded text-amber-600 focus:ring-amber-500 w-4 h-4" /> 
                Remove scratches
              </label>
              <label className="flex items-center mb-4 text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                <input type="checkbox" checked={options.enhanceFaces} onChange={(e) => setOptions({...options, enhanceFaces: e.target.checked})} disabled={status === "processing"} className="mr-3 rounded text-amber-600 focus:ring-amber-500 w-4 h-4" /> 
                Enhance faces
              </label>
              
              {originalFile && status !== "success" && (
                <button onClick={startEnhancing} disabled={status === "processing"} className="w-full mt-2 py-3 bg-amber-600 text-white rounded-xl font-bold hover:bg-amber-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center">
                  {status === "processing" ? (
                    <>
          ...
          <div className="lg:col-span-3 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm h-[600px] flex overflow-hidden relative">
             ...
             {status === "processing" && (
               <div className="m-auto text-center">
                 ...
                 <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">Restoring Photo...</h2>
                  ...
                  <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
                    <button 
                      ...
                      className={cn(
                        ...
                        isZoomed ? "bg-amber-600 hover:bg-amber-700" : "bg-black/50 hover:bg-black/80"
                      )} 
